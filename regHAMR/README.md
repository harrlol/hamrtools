...
## SMACK
Short for "Suite for Modification or Annotation-purposed Cleaning and Keeping", this program takes in an SRR accession list from a given RNA sequencing experiment (along with several other spepifications) and outputs read alignments, HAMR modification tables, rep consensus files, library overlap files, etc. 
USAGE:
```
SMACK.sh <SRA Accession list.txt> <trim requirement T/F> <repo dir> <genome.fasta> 
        <annotation.gff3> <sam filter.pl> <hamr model.Rdata> <read length> <name table.csv>
        <ant lib> <out dir> <#cores> <genome len>
```
```SRA Accession list.txt```: # A list of SRR accession numbers generated from SRA run selector tool, usually represent the sequencing data of an entire experiment, example in demo
...
```genome.fasta```: The genome file of your model organism

```annotation.gff3```: The annotation file of your model organism (recommend same source to genome)

```sam filter.pl```: The provided filtering file

```hamr model.Rdata```: The provided trained model for HAMR

```read length```: The length of your reads (usually ranging from 50~200)

```name table.csv```: A table corresponding each SRR file to the actual condition or sample name of each sequencing data, for the ease of downstream analysis, example in demo

```ant lib```: A folder with annotation libraries of your model organism as generated by running [Diep's script], example in demo

```out dir```: Your desired output directory

```#cores```: Number of threads this process can use

```genome len```: Length/size of the genome of your model organism
...
NOTE in these analyses, the term overlap type might be mentioned. This refers to the kind of library you wish to use to select HAMR modifications for analysis. For example, the threeUTR type selects all HAMR mods that were predicted that lie within the 3' UTR of any given transcript as per indicated in the gtf file provided. The available types include: gene, threeUTR, fiveUTR, CDS, primarymRNA, ncRNA, exon. (case sensitive)

In case of script failure, you can also locate the error via messages/warnings shown on console. The readme for individual programs is also provided below for debugging. 


## Constitutient Programs
### fasterq-dumpAdaptor
First, install the relative sratoolkit manually from https://github.com/ncbi/sra-tools/wiki/01.-Downloading-SRA-Toolkit, and import the command into your $PATH. This program was updated to take individual SRR accessions, thus manual use of this script would need an external loop for experiment-sized downloading. 

USAGE: 
```
fasterq-dumpAdaptor.sh <sratoolkit dir> <SRR number> <out dir> <T/F> <cores>
```
```SRR number```: the SRR accession number to a fastq file
...
### fastq2hamr
A shell wrapper program that takes in an adaptor-trimmed fastq file all the way to applying HAMR, yielding intermediates and hamr prediction table. This is representative of the operational pipeline used to analyze data generated from typical RNA-seq experiments. The pipeline includes the preparing/cleaning steps recommended by Vandivier et al. (https://link.springer.com/protocol/10.1007/978-1-4939-8808-2_4)

USAGE: 
```
fastq2hamr.sh <sample.fastq> <anotation.gtf/gff3> <bowtie dir> <genome.fasta> <sorting_script.pl> <hamr_model.Rdata> <out dir> <mismatch_num> <filename dict> <#cores> <genome len> <repo>
```
...
```anotation.gff3```: these are specific to the model organism you are working with and can be obtained through various databanks, a simple googling should work.
...
```filename dict```: the csv file that pairs each SRR accession number to its corresponding filename

```#cores```: Number of threads this process can use

```genome len```: Length/size of the genome of your model organism

```repo```: a directory containing needed programs
...
e.g. "MyDir/MySubDir" is good, "MyDir/MySubDir/" is bad.
...
USAGE: 
```
consensusOverlap.sh <consensus.bed> <cds.bed> <5utr.bed> <3utr.bed> <gene.bed> <mrna.bed> <exon.bed> <ncRNA.bed> <out dir>
```
...
```cds.bed | threeutr.bed | gene.bed | mrna.bed | exon.bed | ncRNA.bed```: annotated coding region for a given organism, obtainable via gff3 files. Please see https://github.com/dtrain16/NGS-scripts/blob/master/TAIR10_annotation.sh for example.
...
### depth_helper
Script to find the sequencing depth at each hamr-predicted modification position by sunning samtools with corresponding alignment pipeline files. 

USAGE: 
```
depth_helper.sh <out dir> <consensus file basename> <consensus file directory name> <consensus file basename no prefix> <consensus file>
```
Please see code in SMACK.sh for generating these parameters. This script is not designed for manual use.
...
Takes in a longdf file generated by allLapPrep and returns 2 plots per each overlap type that visualizes the abundancy by counts of each type of mod in each experimental group. One plot is by group, the other plot is by mod type.

USAGE:
```
Rscript abundByLap.R <longdf.csv> <out dir>
```
...
USAGE:
```
Rscript distToKnownMod.R <longdf.csv> <knownmod.csv> <out dir>
```
