...
exon <- subset(gff, gff$feature == 'exon') %>%
  ...
  mutate(bio=gene$bio[match(Gene, gene$Name)]) %>%
  select(c('seqname','start','end','bio','Name','score','strand'))%>%
  na.omit()
...
cds <- subset(gff, feature == "CDS") %>% 
  ...
  subset(Isoform==1) %>%
  group_by(seqname, strand, id, feature, score) %>%
  summarise(start = min(start), end = max(end) ) %>%
  mutate(Gene=sapply(strsplit(id, "\\."), function(l) l[1])) %>%
  mutate(bio=gene$bio[match(Gene, gene$Name)]) %>%
  ungroup()%>%
  select(seqname, start, end, bio, id, score, strand) %>%
