...
# assuming V5 is isoform
keepprimary <- function(annotation) {
  out <- data.frame(annotation[grepl(".1", annotation$V5, fixed = TRUE),])
}
...
df <- fread(args[1])
dir <- dirname(args[1])
fiveU <- keepprimary(fread(args[2]))
cds <- keepprimary(fread(args[3]))
threeU <- keepprimary(fread(args[4]))
